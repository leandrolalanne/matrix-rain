...
The user-facing story is in [the README](../README.md). This is the engineering
one: why a browser was not good enough, what made a single-pass port possible,
what was measured, and what is deliberately missing.
...
| `redpill`, in place | 56, moved to plane 16 | 2 | `Matrix Code Terminal` in the chain |
| `redpill`, in a window | 56, the font's own codepoints | 2 | `Matrix-Code` as the primary |
...
The film's glyphs only mean anything if the terminal is actually using them, and
...
So `redpill` checks first, and takes whichever path is open:
...
`redpill operator` carries over what makes that version what it is, taking the numbers
...
`MATRIX_PALETTE=classic` gives you upstream's, unchanged.
...
`install.sh` copies the **whole** tree, and a plugin manager that clones the repo
would too, so everything here lands on a user's machine. Nothing is redundant,
but it helps to know what is what:
